...
with open("../target_words.txt") as f:
    targets = list(sorted(f.read().split(",")))
data = {}
    ...
    def create_data(fn):
        ...
        with open(fn) as f:
            for i, line in enumerate(f):
                for w in re.findall("\w+", line.lower()):
                    words.append((w, i//45+1))
        results = []
        for target in targets:
            for i, (word, page) in enumerate(words):
                if word == target:
                    results.append([*(words[i+j][0] for j in range(-2, 3)), page])
        data[fn] = results
...
def word_get_handler(args):
    ...
    if word_index < len(data[filename]):
        ...
        rep = "\n#{0}: {1} - {2}\n\n".format(word_index+1,' '.join(word_info[0:5]), word_info[5])
    else:
        rep = "\n no more words\n\n"
